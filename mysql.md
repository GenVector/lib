...
###mysql
	步骤:
		...
		4、修改mysql密码:mysqladmin -uroot -p123456 password newPassword
    注意:
    	两个不同的mysql container会造成冲突
    	docker run --net=host 和-p有冲突。如果使用宿主机IP的话,就无法进行端口转发
...
###	1、事务
		...
		事务的隔离级别
			...
			3、可重复读REPEATABLE READ:
				...
				幻读:其他事务的修改不可见。然而优先于此事务之前发生的事务,新插入的数据对于事务可见。
			4、可串行化:
...
####	锁的类型:共享锁(读锁)、排他锁(读锁)
			共享锁:可以申请好多把,所有共享锁的事务可以对锁定数据进行读取操作,不能进行增删操作。如果数据行已经添加共享锁,无法继续添加排他锁
				加锁操作:SELECT ... LOCK IN SHARE MODE
...
####	显式锁定:INNODB利用语句进行显示锁定
			...
			SELECT ... LOCK IN SHARE MODE 对行加共享锁
			SELECT ... FOR UPDATE
...
###	5、事务的实现原理:MVCC:多版本并发控制
		...
		INNODB的MVCC实现(了解)
			MVCC只在REPEATABLE READ(可重复读)和READ COMMITTED(提交读)两个隔离级别下工作。其他两个隔离级别都和MVCC不兼容
				InnoDB的MVCC，是通过在每行记录后面保存两个隐藏的列来实现的。这两个列，一个保存了行的创建时间，一个保存行的过期时间（或删除时间）。当然存储的并不是实际的时间值，而是系统版本号（system version number）。
				每开始一个新的事务，系统版本号都会自动递增。事务开始时刻的系统版本号会作为事务的版本号，用来和查询到的每行记录的版本号进行比较。
				同时MVCC会根据版本号和世间好进行必要的回滚操作。
...
###	1、INNODB:
		...
		5.6之后引入了全文索引。
		5.7目前也可以对TEXT、LONGTEXT字段建立前缀索引
		对于自增序列,innoDb必须作为KEY,否则无法建立和新增。
###	2、MYISAM:
		提供了大量的特性，包括全文索引、压缩、空间函数（GIS）等;对于TXT以及BLOB字段也可以基于前500字符创建索引。
		存储结构为堆表。索引文件和数据文件分别存储。聚簇索引和普通索引结构没什么不同
...
###	索引的类型
####	1、NORMAL 	普通索引
			无法直接在TEXT、BLOB字段创建普通索引(需要制定长度,前缀索引)
####	2、SPATIAL	空间索引
####	3、UNIQUE 	唯一索引
####	4、FULLTEXT	全文索引 
			可直接为varchar、char、text类型创建全文索引。
...
####	6、按主键顺序插入行
			1、UUID占用大量的存储空间
			2、使得聚簇索引插入完全随机,数据排列规则遭到了破坏。索引效率降低。顺序索引失效
###	聚簇索引
...
###	ORDER排序优化:
	排序的原理:mysql 是文件型数据库。索引查询和排序都是在file里做的。select 和order 都是可以保证幂等的。无论做多少次操作,返回的结果集和顺序一致。
	利用索引排序可以避免文件排序和临时表的生成。对最后生成的子临时集排序是在内存中做的(所以是随机的,每次顺序都不一致)。
